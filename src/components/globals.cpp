...
namespace globals
{
	...
	void setup_exe_module()
	{
		...
		if (!GetModuleInformation(GetCurrentProcess(), exe_hmodule, &moduleInfo, sizeof(moduleInfo))) {
			common::log("Globals", std::format("Failed to get exe module information. Error: (0x{:X})", GetLastError()), common::LOG_TYPE::LOG_TYPE_DEFAULT, false);
		} else {
	...
	bool detoured_com_timestamped_log_origin = false;
