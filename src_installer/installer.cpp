...
const wchar_t* installer_name = L"Portal2-Remix-Installer/1.0";
const wchar_t* user_agent = L"User-Agent: Portal2-Remix-Installer";
const char* file_dialog_filter = "Portal2 Executable\0portal2.exe\0All Files\0*.*\0";
const char* file_dialog_title = "Select your portal2.exe";

const char* wiki_link = "https://github.com/xoxor4d/p2-rtx/wiki";
const std::string comp_mod_zip_name = "Portal2-Remix-CompatibilityMod";
const wchar_t* comp_mod_zip_name_wide = L"Portal2-Remix-CompatibilityMod";
#define COMP_MOD_FOLDER "portal2-rtx"
const char* comp_mod_folder = "portal2-rtx";
const char* comp_mod_dll = "p2-rtx.dll";
...
const RemoteModConfig base_mod =
{
	...
	"https://github.com/xoxor4d/p2-rtx-base-mod/archive/refs/heads/master.zip",
	"p2-rtx-base-mod-master/mods",
	"mods",
	"portal2rtx",
	"portal2rtx_commit.txt",
	"portal2rtx_delivery.txt",
...
std::string get_latest_github_commit_sha(const std::string& owner, const std::string& repo, const std::string& branch = "main")
{
	...
	HINTERNET session = WinHttpOpen(installer_name, WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
	if (!session) {
		return "";
	}
	...
	WinHttpAddRequestHeaders(request, user_agent, -1, WINHTTP_ADDREQ_FLAG_ADD);
...
int main()
{
	...
	log_default(true);
	std::cout << PAD << "Portal 2 - RTX Remix Compatibility Mod Installer (https://github.com/xoxor4d/p2-rtx)\n";
	log_default();
	std::cout << PAD << "- Make sure you've placed both the installer and the 'Portal2-Remix-CompatibilityMod-X.X.X.zip' into the same folder.\n";
	std::cout << PAD << "- If installation fails, refer to the release page on GitHub for manual install instructions.\n\n\n";

	std::cout << PAD << "Select your portal2.exe inside the Portal 2 directory to continue ...\n";
	Sleep(500);

    std::string exe_path = open_file_dialog();
	if (exe_path.empty()) 
	{
    ...
    const std::string game_dir = std::filesystem::path(exe_path).parent_path().string();
	...
	bool has_remix_comp_mod = file_exists(game_dir + "\\bin\\d3d9.dll") && file_exists(game_dir + "\\" + comp_mod_dll);
	...
	if (!skip_rtx_comp_install)
	{
		...
		static const wchar_t* zip_prefix = comp_mod_zip_name_wide;
		static const std::string zip_prefix_str = comp_mod_zip_name;
		...
		for (const auto& entry : std::filesystem::directory_iterator(get_installer_dir()))
		{
			...
			{
				// Extract version from filename: XYZ-Remix-CompatibilityMod-X.Y.Z.zip
		...
		if (found_zip.empty())
		{
			log_error("Could not find any zip starting with '" + comp_mod_zip_name + "'.");
		...
		has_remix_comp_mod = file_exists(game_dir + "\\bin\\d3d9.dll") && file_exists(game_dir + "\\" + comp_mod_dll);
		...
		if (!has_remix_comp_mod)
		{
			...
			MessageBoxA(nullptr, "Running the game via Steam is NOT supported!\nRun the game using run-p2-rtx.bat.\n", "IMPORTANT", MB_OK | MB_ICONWARNING);
		}

		// backup some files

		if (MoveFileExA(
			(game_dir + "\\" COMP_MOD_FOLDER "\\comp_settings.toml").c_str(),
			(game_dir + "\\" COMP_MOD_FOLDER "\\comp_settings.toml.bak").c_str(),
		...
		if (!extract_zip(found_zip, game_dir, comp_mod_zip_name))
		{
			log_error(
				"Failed to extract '" + comp_mod_zip_name + "' files from '" + comp_mod_zip_name + ".zip'\n"
	...
	// Only prompt about DirectX if this is a fresh install
